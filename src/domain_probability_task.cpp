...
bool task_compute_domain_probabilities_per_phylum(
	...
	for (auto& phylum : phyla) {
		...
		string phylum_dir = phylumFolder + phylum_lower + "/";

		filesystem::create_directory(phylum_dir);
		...
		xt::xarray<double> uniform_prior = xt::eval(xt::ones<double>({n_assemblies}) / n_assemblies);
		...
		for (auto& domain : protein_domains) {
			...
			for (int ix = 0; ix < n_assemblies; ++ix) {
...
void compute_domain_probabilities(
	...
	cerr << "Processing " << phyla.size() << " phyla" << endl;
	cerr << "Starting " << n_threads << " threads" << endl;

	n_per_thread = ceil((double) phyla.size() / (double) n_threads);
